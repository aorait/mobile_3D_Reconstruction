...
void adjustBundle(
        ...
        cv::Mat             &intrinsics,
        vector<vector<cv::Point2f>> & image2dFeatures) {

    cout<<"ENTERED BA"<<endl;
    ...
    for (size_t i = 0; i < cameraPoses.size(); i++) {
        cv::Mat pose = cameraPoses[i];
        cv::Vec3d t(pose.at<double>(0, 3), pose.at<double>(1, 3), pose.at<double>(2, 3));
        cv::Matx33f R,R_trans;
        pose(cv::Rect(0,0,pose.cols-1,pose.rows)).copyTo(R);
        cv::transpose(R,R_trans);
        double angleAxis[3];
        ceres::RotationMatrixToAngleAxis<double>(*R_trans, angleAxis); //Ceres assumes col-major...
    ...
    double focal = intrinsics.at<float>(0, 0);

    vector<cv::Vec3d> points3d(pointCloud.rows);

    for (int k = 0; k < pointCloud.rows; k++) {
        points3d[i]=pointCloud.row[k];

        for (int j=0;j<image2dFeatures.size();++j) {
            //kv.first  = camera index
            //kv.second = 2d feature index
            cv::Point2f p2d = image2dFeatures[k].row[j];

            //subtract center of projection, since the optimizer doesn't know what it is
            p2d.x -= intrinsics.at<float>(0, 2);
            p2d.y -= intrinsics.at<float>(1, 2);
            ...
            // cout<<cameraPoses6d[kv.first].val<<endl;
            // cout<<"points 3d "<<endl;
            // cout<<points3d[i].val<<endl;
...
void visualize_epipolar_lines(cv::Mat im_left, std::vector<cv::Point2f> points_left,
                              cv::Mat im_right, std::vector<cv::Point2f> points_right,
                              std::vector<cv::Vec3f> lines, std::string im_name="left") {
    // Convert to color image
    cv::cvtColor(im_left, im_left, cv::COLOR_GRAY2BGR);
    int width = im_left.cols, height = im_left.rows;
    ...
    for (int i = 0; i < lines.size(); ++i) {
        ...
        int c3 = std::rand() % 256;
        cur_line = lines[i];
        pt_left = points_left[i];
        pt_right = points_right[i];

        // the point
        cv::circle(im_left, pt_left, 3, cv::Scalar(c1, c2, c3), 2);
        // the line
        cv::line(im_left, cv::Point(0, (int)(-cur_line[2]/cur_line[1])),
                 cv::Point(width, (int)(-(cur_line[2] + cur_line[0] * width) / cur_line[1])),
                 cv::Scalar(c1, c2, c3));
    }
    display_image(im_left, im_name + " image epipolar lines", true);
}
...
void computeProjectionMats(cv::Mat &K, vector<cv::Mat> &transformedMats, vector<cv::Mat> &proj_BA){
    for(int i=0; i<transformedMats.size();++i){
        cv::Mat temp;
        temp=K*transformedMats[i];
        proj_BA.push_back(temp);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    vector<cv::Mat> projMats,transformedMats, proj_BA;
    string num=argv[2];
    ...
    computeProjectionMats(mK,transformedMats,proj_BA);
    // cout<<"overall projection matrices"<<endl;
    //     for(int i=0;i<proj_BA.size();++i){
    //     cout<<proj_BA[i]<<endl;
    // }
    cv::waitKey();
    return 0;   
